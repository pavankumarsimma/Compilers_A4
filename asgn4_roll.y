...
init_declarator_list:
                init_declarator { yyinfo("init_declarator_list -> init_declarator"); }
                | init_declarator_list COMMA init_declarator
                { yyinfo("init_declarator_list -> init_declarator_list , init_declarator"); }
                ;

init_declarator:
                declarator{ yyinfo("init_declarator -> declarator"); }
...
identifier_opt:
                IDENTIFIER { yyinfo("identifier_opt -> identifier"); 
                printf("\t\t\t\tidentifier = %s\n", $1); }
...
enumerator:
            IDENTIFIER 
            { yyinfo("enumerator -> enumeration_const"); 
            printf("\t\t\tenumeration_const = %s\n", $1); }
            | IDENTIFIER ASSIGNMENT constant_expression
            { yyinfo("enumerator ->enumeration_const = constant_expression"); 
            printf("\t\t\tenumeration_const= %s\n", $1); }
            ;
...
direct_declarator:
                IDENTIFIER 
                { yyinfo("direct_declarator -> identifer");
                    printf("\t\t\t\tidentifier = %s\n", $1); }
...
identifier_list:
                IDENTIFIER 
                { yyinfo("identifier_list -> identifer"); 
                    printf("\t\t\t\tidentifier = %s\n", $1); }
...
initialiser:
        assignment_expression { yyinfo("initialiser -> assignment_expression"); }
...
jump_statement:
            ...
            { yyinfo("jump_statement -> goto identifier ;"); 
                printf("\t\t\t\tidentifier = %s\n", $2); }    
            | CONTINUE SEMI_COLON { yyinfo("jump_statement -> continue ;"); }
            | BREAK SEMI_COLON
